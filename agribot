...
from tensorflow.keras.utils import to_categorical  # Changed this
...
# Create the directory 'saved_state' if it doesn't exist
...
# Save the CountVectorizer
    ...
    y = labelencoder_intent.fit_transform(y)  # Transform y using LabelEncoder
    y = to_categorical(y)  # Use to_categorical directly
...
#     X = cv.fit_transform(X.astype('U')).toarray()
    ...
    # Save the entity classifier mod
...
while True:
    ...
    print(f"Predicted Intent Raw Output: {predicted_Intent}")

# Get the index of the highest probability
    result = np.argmax(predicted_Intent, axis=1)[0]
    print(f"Predicted Intent Index: {result}")

# Check the USER_INTENT mapping
    USER_INTENT = None
...
# Now check if the correct response is being retrieved
    if USER_INTENT:
